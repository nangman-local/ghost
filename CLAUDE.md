...
- **실기기 수동 검증 완료 (2026-09-14, 정호):** `npm start`로 10~20분 실사용 테스트. 클릭 통과(캐릭터 밖 클릭 시 뒤쪽 앱 반응), 캐릭터 클릭 애니메이션·드래그, 캡처 도구(`SnippingTool.exe`) 사용 후 캐릭터 topmost 복구, 다른 창 최대화 상태에서도 캐릭터 표시, 메모장 타이핑 중 포커스 유지(한 글자씩 정상 입력 확인) 전부 정상. 활성 창 감지에 카카오톡도 새로 확인됨(VS Code는 아직 미확인). 세션 동안 크래시나 예외 없음. README 체크리스트 반영 완료

### 미확인 (실기기 수동 검증 필요)
- (관찰만) 전체화면 영상/게임, 배율 125%/150%, 다중 모니터, CPU 사용량 — 아직 테스트 안 함
- VS Code 전환 감지
...
1. [x] README 체크리스트로 실기기 수동 검증 → 결과를 이 파일에 반영 (2026-09-14, 관찰 항목 3개 제외 전부 완료)
