...
**감지·오버레이·상태머신 변경은 Android 코어 오너 리뷰 후** 머지합니다.
...
그 외에 문서 갱신이 정말 불필요하면 PR 본문 **줄 맨 앞에** 아래 마커와 사유를 적습니다.

```
NO-DOCS-NEEDED: 오타 수정이라 제약·결정사항 변화 없음
```

**줄 맨 앞에서 시작해야 하고(들여쓰기 불가), 실제 사유가 있어야 합니다.** 아래는 인정하지 않습니다.

- 마커만 적고 사유 없음
- 문장 중간에 섞여 있음 / 들여쓰여 있음
- `<사유>` 같은 플레이스홀더를 그대로 둠
- **코드블록(``` ```) 안에 있음** — 이 규칙을 문서화하는 PR이 예시만으로 스스로 면제되지 않도록 코드블록은 검사에서 제외합니다

> 예전에는 본문에서 "문서 변경이 불필요" 같은 자연어를 찾았는데, **PR 템플릿에 그 문구가 들어 있어서 템플릿을 그대로 쓴 모든 PR이 검사를 건너뛰었습니다.** 그래서 의도적으로 적어야만 걸리는 마커로 바꿨습니다.
