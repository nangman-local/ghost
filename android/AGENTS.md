...
전체 인수 검증은 남아 있다 — 미완료 항목은 #2(Chrome 페이지 제목 직접 감지) · #3(탭 인사 말풍선). #1(인수 체크리스트 8개)은 완료됐다.
특히 **Chrome 페이지 제목 직접 감지는 일부 조건에서 감지에 실패한다(#2).** 일반 페이지와 시크릿 모드는 감지되지만
새 탭·주소창 편집 중·전체 화면·커스텀 탭은 감지되지 않는다. 실기기에서 관측된 `Chrome: Google` 같은 값은 창 제목 대체값이며,
페이지 제목으로 가장하지 않는다.
