...
- Chrome 버전/화면 구조에 따라 주소창 숨김, 새 탭, 전체 화면, 커스텀 탭에서 값이 없을 수 있다. 2026-09-23 검증에서 **새 탭·주소창 편집 중·전체 화면·커스텀 탭은 실제로 감지되지 않았다**(#2). 시크릿 모드는 감지된다. 제목/주소의 원자적 동일 페이지 보장이나 모든 Chrome 버전 지원은 미검증이다. 광범위 본문 수집으로 우회하지 않는다.
...
- 2026-09-23 삼성 실기기에서 Chrome 조건별 감지를 검증했고 **일부 조건에서 감지에 실패했다.** 일반 페이지와 시크릿 모드는 감지되지만 **새 탭·주소창 편집 중·전체 화면·커스텀 탭은 감지되지 않는다.** 후속 수정 대상이며 진행 상황은 #2에서 추적한다.
